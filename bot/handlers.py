...
async def check_access_and_analyze(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Список всех вопросов компании
    questions = session.query(Question).filter_by(company_id=company.id).all()

    user_text = (
            "Анализируй эти отзывы:\n"
            + reviews_str +
            f"\nОпредели тональность, основные проблемы и рекомендации, и напиши примерное количесво пользователей равное {str((len(all_answers))/(len(questions)))}"
    )
